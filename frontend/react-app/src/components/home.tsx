...
callme()
  ...
  const filt = ()=>{
    setMyData2(
         myData.filter((d)=>{
   if(d.title.toLocaleLowerCase().trim().includes(search.toLocaleLowerCase().trim()))
   {
    return d;
   }
   })
    ...
    <>
      <div className="main">
        <nav>
          ...
          <input type="text" placeholder="Search" className="search_input" onChange={(e)=>{setSearch(e.target.value)}} />
  ...
  {JSON.parse(localStorage.getItem("userInformation")) ? (
    ...
    {JSON.parse(localStorage.getItem("userInformation"))?  <a href={JSON.parse(localStorage.getItem("userInformation")).name == "Guest"?"/":"/orders"} className="a_orders_2">Orders  </a>:  <a href={"/"} className="a_orders_2">Orders  </a>     }    
            ...
            <div className="basket">
              ...
              <p>
                {cartFromDbb?cartFromDbb.length:0 }
              </p>
            </div>
          </div>
